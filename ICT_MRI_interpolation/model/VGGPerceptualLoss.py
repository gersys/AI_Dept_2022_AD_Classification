...
class VGGPerceptualLoss(torch.nn.Module):
    def __init__(self, resize=True):
        ...
        blocks.append(torchvision.models.vgg19(pretrained=True).features[14:23].eval())
...
class VGGPerceptualLoss_FILM(torch.nn.Module):
    ...
    def forward(self, input, target, feature_layers=[0, 1, 2, 3, 4], style_layers=[0, 1, 2, 3, 4]):
        if input.shape[1] != 3:
            input = input.repeat(1, 3, 1, 1)
            target = target.repeat(1, 3, 1, 1)
        input = (input-self.mean) / self.std
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in style_layers:
                ...
                gram_x = act_x @ act_x.permute(0, 2, 1)
                gram_y = act_y @ act_y.permute(0, 2, 1)
                g_loss += torch.nn.functional.l1_loss(gram_x, gram_y)
